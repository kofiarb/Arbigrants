...
NAME,
LAST_VALUE(TVL) OVER (PARTITION BY DATE_TRUNC('week', DATE), NAME ORDER BY DATE) AS TVL
FROM (
    SELECT
    DATE,
    NAME,
...
QUALIFY ROW_NUMBER() OVER (PARTITION BY DATE_TRUNC('week', DATE), NAME ORDER BY DATE DESC) = 1
)
