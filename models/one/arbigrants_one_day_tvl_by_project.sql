...
NAME,
LAST_VALUE(TVL) OVER (PARTITION BY DATE_TRUNC('day', DATE), NAME ORDER BY DATE) AS TVL
FROM (
SELECT 
DATE,
m.NAME,
...
GROUP BY 1,2)
QUALIFY ROW_NUMBER() OVER (PARTITION BY DATE_TRUNC('day', DATE), NAME ORDER BY DATE DESC) = 1
)
...
QUALIFY ROW_NUMBER() OVER (PARTITION BY DATE_TRUNC('day', DATE) ORDER BY HOUR DESC) = 1
)
